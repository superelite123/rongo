...
class SaisonCaptureResponseDto extends MdkBaseDto
{
    ...
    /**
     * trAd用の詳細結果コードを取得する<br>
     * @return string trAd用の詳細結果コード<br>
     */
    public function getTradVResultCode() {
        $processor = new TGMDK_ResElementProcessor($this->__toString());
        $saisonUrl = $processor->get_trad_url();
        $cardUrl = $this->getCardTradUrl();
        if ($saisonUrl != null && 0 < strlen($saisonUrl)) {
        // SaisonのtradUrlがある場合
            return $this->vResultCode;
        } else if ($cardUrl != null && 0 < strlen($cardUrl)) {
        // SaisonのtradUrlが無く、カードのtradUrlがある場合
            return $this->cardVResultCode;
        }
        // どちらも無い場合
        return $this->vResultCode;
     ...
     * @throws {@link RuntimeException#RuntimeException(Throwable)}
     */
    public function getTradUrl() {
        // SaisonのtradUrlを取得
        $processor = new TGMDK_ResElementProcessor($this->__toString());
        $url = $processor->get_trad_url();

        // SaisonのtradUrlが無い場合はカードのtradUrlを取得
        if ($url == null || strlen($url) <= 0){
            $url = $this->getCardTradUrl();
        }
        return $url;
    }
